...
class CommittedInverseNodeReferenceRepresentationReadConverter: Converter<Document, CommittedInverseNodeReferenceRepresentation> {

    val invConverter = InverseNodeReferenceRepresentationReadConverter()

    override fun convert(doc: Document): CommittedInverseNodeReferenceRepresentation {
        val repr = invConverter.convert(doc)
        val revision = doc.getString("revision")
        return DefaultCommittedInverseNodeReferenceRepresentation(RevisionNumber(revision), repr)
    }
